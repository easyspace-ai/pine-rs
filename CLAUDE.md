...
> 本文件是 Claude Code 的**唯一入口文件**。每次会话开始时自动读取。
...
- **开始新 Phase** 前：`/clear`，然后重新读取 CLAUDE.md
...
运行多个 Claude 会话并行可以加速开发——使用 Writer/Reviewer 模式：一个 Agent 写代码，另一个审查；或者一个写测试，另一个写通过测试的代码。
...
*pine-rs CLAUDE.md v1.0 — 与 docs/GUIDE.md 配套使用*
